...
const Card = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://192.168.156.69:3000/getUsers', {
  ...
  const likeUser = async (likedUserId) => {
    try {
      const response = await axios.post('http://192.168.156.69:3000/like', { likedUserId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const dislikeUser = async (dislikedUserId) => {
    try {
      await axios.post('http://192.168.156.69:3000/dislike', { dislikedUserId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } catch (error) {
